...
wss.on('connection', (ws) => {
  ws.send(JSON.stringify({ type: 'snapshot', books: Array.from(latestByTopic.values()) }));
});
...
async function consume() {
  ...
  await consumer.subscribe({ topics: [/^.+-(asks|bids)$/], fromBeginning: true });
  ...
  await consumer.run({
    eachMessage: async ({ topic, message }) => {
      ...
      try {
        ...
        console.error(`Skipping bad message on ${topic}: ${err.message}`);
      }
    },
  });
}
...
consume().catch((err) => {
  console.error(`Kafka consumer error (UI stays up; ensure broker at ${KAFKA_BROKER} is reachable): ${err.message}`);
});
