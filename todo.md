...
- [x] ~~`scripts/diagnose-stuck-markets.sh` declares `WINDOW_MS` and documents it as "how far back to read", but never uses it — the script always reads `--from-beginning`. Misleading in a tool reached for during an incident. It also consumes each dead-letter topic twice (8s timeout each) to get the summary and then the last reason; one pass gives both~~ — **moot: the script was DELETED 2026-09-12 at the user's request, never having been run**
...
- [x] ~~`scripts/diagnose-stuck-markets.sh` (2026-08-19) reads `control-plane` and every `-rejected-flink` topic together and flags markets whose last rejection is `awaiting_snapshot`/`no_baseline` — the deadlock fingerprint. **Syntax-checked only, never executed** (docker was down); run it against the live stack to confirm whether any market is still wedged from before the fix~~ — **moot: the script was DELETED 2026-09-12 at the user's request, never having been run**
...
- [x] **Depth dropdown — the level limit is applied on the BACKEND** (2026-09-13, user request, branch `feat/web-service-changes`) — a third dropdown picks 25/50/100/200 levels per side, default from `LEVEL_LIMIT` in `.env` (25). The cut is made in the hub **at fan-out**, not at ingest: `hub.latest` keeps the whole book so two browsers can watch it at different depths, `Publish` now builds its `WSUpdate` inside the client loop instead of sharing one message, and both it and `selectBooks` go through `domain.Book.Limit(n)` (returns a copy, keeps the FIRST n — the best n only because every producer emits asks ascending and bids descending). The depth is NOT part of `Selection` (that doubles as the book key); it rides on `WSSelect.limit` and `client.limit`. The choices and the default are **served in the `catalog` message** rather than duplicated in the JS — the opposite of the `AGGREGATED`/`MERGED` constants, and a working precedent for the item above. Invalid depths are replaced and logged, never honoured, at both gates (`config` for the env var, `hub.limitOr` for a client request) — "no limit" must not be reachable by asking for a bad one. `LEVEL_LIMIT` added to `web/.env.example` and to the `web` service in **both** compose files. `make check` green; 14 new tests, mutation-verified (`Book.Limit` as a no-op fails 11). **Not run against live Kafka** — no real deep book has been cut yet. See [[project_orderbook_viewer]]
- [ ] **Verify the depth limit against a live book** (opened 2026-09-13) — everything so far used synthesised 300-level books. Watch that a real `p{id}-{side}` record is cut to the dropdown value, that switching depth repaints from the `snapshot` reply, and that the `ws select: … N level(s)` line reports what the browser actually chose
- [x] **Level counts moved into the header, `event_time` dropped from them** (2026-09-13, user request, branch `feat/web-service-changes`) — `#meta` is a `<span>` in `<header>` beside the dropdowns instead of a `<div>` under the book, showing only `N asks · N bids`. `event_time` still arrives on every book over the websocket; it is just no longer rendered anywhere (the memory note on its timezone handling is kept for whoever displays it next). CSS-and-markup only, no server change
- [x] **All three dropdown defaults configurable** (2026-09-13, user request, branch `feat/web-service-changes`) — `DEFAULT_PAIR=BTC/USDT`, `DEFAULT_EXCHANGE=okx|aggregated|merged`, `DEFAULT_LEVEL_LIMIT=25` (the last renamed from `LEVEL_LIMIT`, added hours earlier the same day, so the three share a family). **Names, not ids, in `.env`** (user's choice — an id there says nothing without the DB open beside it); the registry resolves them against postgres on every refresh and the browser still receives ids on the catalog. `registry.Catalog()` now fills the WHOLE catalog message, depths included, and `hub.SetCatalog` stamps nothing. Unresolvable pair → 0 → the page falls back to the first market; unknown exchange → aggregated. `selectedExchange` starts `null` in the page, which is what lets a configured default apply on first load. One log line per var on the transition only (verified: 26s with postgres down prints it once, not three times). `make check` green, 8 new tests, mutation-verified. See [[project_orderbook_viewer]]
- [ ] **`DEFAULT_PAIR`/`DEFAULT_EXCHANGE` have never been resolved against a real postgres** (opened 2026-09-13) — docker was down, so every test used the fake repo. Check that `BTC/USDT` matches how the server DB actually spells its `currencies.name` rows, and that an exchange name matches `exchanges.name` and not the Persian `label`
- [x] **Exchange dropdown says "separated", not "aggregated"** (2026-09-13, user request, branch `feat/web-service-changes`) — `All exchanges (separated)`, and `DEFAULT_EXCHANGE` now takes `separated` only (user's call; `aggregated` reaches the same view through the unknown-name fallback, so nothing breaks). `domain.AggregatedName` → `SeparatedName`. **Nothing on the wire changed**: `AggregatedExchangeID`, `Book.ExchangeID()`, the `aggregatedPattern` regex, the `AggregatedOrderBookEvent` schema and job 6 keep the pipeline's word — the two words name the same view, one from the reader's side and one from the job's. See [[project_orderbook_viewer]]
- [x] **Exchange dropdown: brand-spelled names, sorted, with the views below a rule; merged shows a count** (2026-09-13, user request, branch `feat/web-service-changes`) — `postgres/02_seed.sql` renames every `exchanges.name` to its English brand spelling (`Nobitex`, `Bitpin`, `Wallex`, `Ramzinex`, `Bitget`, `Bybit`, `OMPFinex`, `OKX`, `LBank`); the dropdown lists exchanges A–Z, then a separator (`disabled <option>`, not `<hr>`), then `merged`/`separated` A–Z; a merged level now shows the COUNT of contributing exchanges and **nothing** when only one contributed (reverses the 2026-08-24 comma-joined list, also a user choice). Alignment fixed with `table-layout: fixed` + a 120px exchange column — asks and bids are two separate tables and were sizing that column independently. See [[project_orderbook_viewer]]
- [ ] **⚠ `exchanges.name` is sent VERBATIM to NiFi — confirm the rename is safe before using the subscription console** (opened 2026-09-13) — `market-subscriptions` posts `{"exchange": name, "market": symbol}` to NiFi's `/subscribe`/`/unsubscribe`, and `csv-bulk-sync/markets.csv`, which has always driven that endpoint, spells every exchange lowercase. If NiFi matches exactly, it will now receive `Bitpin` instead of `bitpin` and match nothing. The flow lives on the server (`nifi/` here is just a Dockerfile), so nothing in this repo can settle it
- [ ] **The renamed exchanges need an `UPDATE` on the live server DB** (opened 2026-09-13) — `02_seed.sql` only runs on a fresh volume, so the server keeps the old lowercase names until someone runs the statements by hand. Same trap as the 2026-08-25 percent columns
- [x] **`web/` renamed to `orderbook-viewer/`** (2026-09-13, user request, branch `feat/web-service-changes`) — the name says what the service is, and matches how its siblings are named. EVERYTHING moved: directory, **Go module** (every import), binary, compose service key + `container_name` + `hostname` in both compose files, the e2e service and project (`orderbook-viewer-e2e`, `ORDERBOOK_VIEWER_E2E_PROJECT`), and the comments elsewhere naming the old path. `make check` green, binary serves, `docker compose config` resolves. ⚠ `docker compose up -d --build web` is now `… orderbook-viewer`. See [[project_orderbook_viewer]]
- [ ] **⚠ Consumer groups are now `orderbook-viewer-agg-*` / `orderbook-viewer-ex-*`** (opened 2026-09-13) — nothing in this repo matched the old `orderbook-web-*` prefix, but a Grafana panel or alert on the SERVER watching that lag will silently show nothing once this deploys. Consumer lag is how the 2026-09-06 hub freeze was meant to be caught, so check before relying on it
- [ ] **Root `README.md` documents this service as `npm i && npm start`** (spotted 2026-09-13 during the rename, NOT fixed — only the path was corrected) — it is a Go app, so the lines should be `cd ../orderbook-web` + `go run .`
- [x] **BUG FIX: exchange dropdown blank, DEFAULT_EXCHANGE ignored** (2026-09-13, user bug report with screenshot, branch `feat/web-service-changes`) — two defects. (1) The separator option carries no `value`, so `o.value === undefined` matched it, and before the first catalog `default_exchange_id` is also `undefined` → `selectedExchange = undefined`, which then kept matching the separator and never recovered (`$el.value = "undefined"` selects nothing). (2) Fixing that exposed a second: the bootstrap `refreshDropdowns()` call settled on the fallback before any catalog, and "keep the current selection" then outranked the configured default. Now one `pickOption()` for all three dropdowns (separators excluded, undefined/null are not values) and the dropdowns are filled ONLY from a catalog. Verified by running the real `pickOption` source, extracted from the HTML, in node. See [[project_orderbook_viewer]]
- [x] **Dropdown defaults are database ids** (2026-09-13, user request, branch `feat/web-service-changes`) — `DEFAULT_PAIR_ID` (a `markets.id`) and `DEFAULT_EXCHANGE_ID` (an `exchanges.id`, or `0` = separated / `-1` = merged), reversing the name-based version of the same day. The platform speaks ids everywhere and the two views already had ids in the `exchange_id` vocabulary, so `domain.SeparatedName`/`MergedName` are deleted — one spelling per concept. `config` parses and reports a non-number; `registry` checks the id exists and logs what it IS (`DEFAULT_EXCHANGE_ID=8 is OKX`), which is what a config file cannot show. Catalog, browser and fallbacks unchanged — already id-based. Mutation-verified. See [[project_orderbook_viewer]]
- [x] **BUG FIX: `orderbook-viewer/.env` had no effect in docker** (2026-09-13, user bug report — set `DEFAULT_EXCHANGE_ID=6`, page still opened separated) — two causes, both mine: the runtime image copies only the binary so the file never reached `/app`, and compose's `environment:` would have overridden it regardless (`godotenv` never overwrites a set variable). Documenting `.env` while compose hardcoded the same keys was the real mistake. Fix: both compose files load `env_file: ./orderbook-viewer/.env` (`required: false`) and the `DEFAULT_*` keys left `environment:`, so ONE file configures `go run .` and the container and a change needs no rebuild. ⚠ `required:` needs Compose v2.24+; on older versions the file must exist. Verified both ways with `docker compose config`. See [[project_orderbook_viewer]]
- [x] **`feat/web-service-changes` merged into `main`** (2026-09-13, user request) — one conflict, `memory/MEMORY.md` (main's new Price Merger § entry vs the branch's `[[orderbook-viewer]]` link rename), kept both. `go vet` + `go test` green in `orderbook-viewer/`. Before deploying: the old `web` container must be removed by hand (`docker compose up -d --remove-orphans`, or `docker rm -f orderbook-web`), since the service key changed and the new one binds the same port 3000
...
## flink/normalizer (job 1 parsers — in `job-parser/` since the 2026-09-19 split)
...
- [ ] **⚠ ex6 scenario 48 NOT RUN LIVE, and the parser change is NOT DEPLOYED.** The docker stack was down and the harness does a destructive `down -v`, so `48-ex6-rest-snapshot-resync` has never executed — the snapshot COUNT and the reset record's event time are reasoned from `Ex6SequenceGap`/`Ex5RestSnapshotResync`, not observed. Deploy is job-1 only (rebuild the jobs image, resubmit **`job-parser`** — written as `job-pair-extractor` before the 2026-09-19 split; no schema change, so no re-registration). Also worth a live mutation check, the way scenario 31 got one: stamp `result.u` as the sequence id and confirm 48 fails
...
- [x] **Diagnosed: merged spread ≠ aggregated spread in the UI** (2026-09-13, user report, read-only on the dev server) — the cause is LAG, not the merge logic. Job 6 writes 908 rec/s, the merger 727 and adjustment 474 (both 1-in/1-out), so their backlog grows without bound. `p1-asks-merged` was ~17 min and `p1-bids-merged` ~26 min behind `p1-{side}`, with the gap growing. The host is CPU-starved (run queue ~30 on 8 CPUs). See [[project_price_merger]] § 2026-09-13
- [ ] **⚠ DECIDE THE FIX for the merger/adjustment backlog** — options not yet chosen: (a) reduce host CPU contention or give the TaskManager more CPU; (b) raise merger/adjustment parallelism, which needs more slots (cluster is 8/8); (c) compute the merged view inside job 6 from the same in-memory book, removing a full Avro decode+encode hop and keeping both views consistent (this reverses the 2026-08-11 "outside `flink/normalizer/`" decision); (d) have the web UI refuse to pair asks and bids whose `event_time`s are far apart. Until then `-merged` and `-adjusted` are minutes stale on the dev server. For (b), the steps and caveats are in [[project_price_merger]] § 2026-09-13 (`PARALLELISM=N ./flink/run-job.sh merger`, cancel the old job first, free slots, direct memory, CPU)
- [x] **Dev TaskManager slots 8 → 12** (2026-09-13, user request) — room for the merger at parallelism 5. `docker compose config -q` is clean. Prod already has 12 (4 × 3)
- [x] **Makefile per-job parallelism** (2026-09-13, user request) — `PARALLELISM_<job>` / `DEFAULT_PARALLELISM := 1`, `PARALLELISM_merger := 5`, one `submit_jobs` helper replacing the 4 loops (prod-deploy included). Verified with a fake `FLINK_RUN` (values passed through, a failure stops the chain); not run against a cluster
- [x] **Merger parallelism 5 VERIFIED LIVE** (2026-09-13) — `-merged` 1052.6 rec/s vs aggregated 1053.4, equal per topic, event_times identical; no further merger parallelism needed. See [[project_price_merger]] § 2026-09-13
- [x] **`orderbook-adjustment` is lagging** (2026-09-13) — 608.6 rec/s out vs 1053.4 in. Addressed in config: `PARALLELISM_adjustment := 3`, merger 5 → 4, 14 dev slots
- [x] **job-aggregator CPU fix** (2026-09-13, user request) — it was the new bottleneck (99.8% busy, source 86% backpressured, ~1000/s against ~1200/s in). Parse-once sort + `GenericData.Record` serializer; `processElement` 177→98 µs, `toGenericRecord` 41→10 µs; byte-identical output and identical order proven against HEAD; `AggregatedOrderBookSerializerTest` added; 22 tests green, 3 mutations killed; `PARALLELISM_job-aggregator := 2`. See [[project_aggregator]] § 2026-09-13
- [ ] **Deploy + verify on the dev server**: recreate `jobmanager` + `taskmanager` (14 slots), then `make run-all-jobs`. Verify all 14 slots used and 8 jobs RUNNING, no `Direct buffer memory` OOM, job-aggregator busy well under 100% with no source backpressure, and `p*-{side}` ≈ 2 × `ex*-p*-orderbook-snapshot-flink` rate, with `-merged`/`-adjusted` matching it
- [x] **`prod-deploy` no longer fit prod** (2026-09-14, user decision) — prod TaskManagers 4 × 3 → **4 × 8 = 32 slots**, process.size 8g / limit 9g unchanged. Parallelism sum is 14. `docker compose -f docker-compose.prod.yml config -q` clean; NOT deployed. See [[project_flink_production]] § 2026-09-14
- [ ] **Prod review follow-ups (2026-09-14, not applied)** — see [[project_flink_production]] § 2026-09-14: slot spreading across TMs, host RAM/CPU budget, restart policy on non-Flink services, the stale "8 jobs in 8 slots" alert text, and dev `docker-compose.yml` comments still saying 14 slots / 2g after `158acf3`
- [x] **NiFi shares its volumes between dev and prod compose** (2026-09-14, user request) — already true (same keys, same project name). Synced prod `NIFI_WEB_PROXY_HOST` (was missing .35), which the start script writes into the shared conf volume. Resolved nifi service is identical in both files. See [[project_flink_production]] § 2026-09-14 NiFi
- [x] **Prod compose sized to the host** (2026-09-14, user request) — 31 GiB / 8 cores measured; 4 TM × 4 slots @ 3g (limit 3500m), Kafka 2G, NiFi 6g. Config valid, NOT deployed. After deploy check `free -g` and that all 4 TMs get tasks. See [[project_flink_production]] § 2026-09-14 FINAL
- [ ] **Prod-shaped server: 4 upstream jobs starved by `NoResourceAvailableException`** (2026-09-14, diagnosed, NOT fixed) — aggregator was submitted at parallelism 8 and book-builder at 4, not the Makefile's 2/1, so 23 slots were requested against 16. Fix: cancel and resubmit with the Makefile values (sum 14), or add slots/memory if 8/4 is wanted. Find out where the override came from. Optional: a pre-submit check that the parallelism sum is ≤ `slots-total`. The Makefile comment still says prod is "4 x 8 = 32" (it is 4 × 4 = 16). See [[project_flink_production]] § 2026-09-14 NoResourceAvailableException
- [x] **`scripts/watch-topic.sh <topic>`** (2026-09-14, user request): live tail of partition/offset/CreateTime for one topic, with a human-readable time in the server timezone; exits 2 on an out-of-order offset. Use instead of Kafka UI live mode. `bash -n` and the usage path were checked; NOT run against a broker yet. See [[project_kafka_topic_strategy]] § 2026-09-14
- [ ] Kafka producer send is ~20% of the aggregator thread: each ~44 KB record exceeds the default `batch.size` (16 KB), so every send allocates an unpooled buffer. Measure before tuning `batch.size`
- [ ] ~~**Deploy + verify merger parallelism 5 on the dev server**~~ (done, see above): recreate `taskmanager` (and `jobmanager`, given the 2026-08-25 slot-leak note), which restarts every job; then `make run-all-jobs` (now submits merger at 5); check for `OutOfMemoryError: Direct buffer memory`; re-measure that `-merged` output ≈ `p*-{side}` output (~908 rec/s) and that job 6 still keeps up
- [ ] Decide on CPU weighting for the dev `taskmanager` (e.g. `cpu_shares: 4096`) and/or capping the outside-repo `alloy` container, which used ~1 core. Nothing has a CPU limit today; see [[project_price_merger]] § 2026-09-13
- [ ] Update the stale "8 slots" comments in `MergerJob`/`AggregatorJob`/`AdjustmentJob` direct-memory notes
- [ ] **ex9/lbank `event_time` is +8 h live** (2026-09-13, measured 28 790 s ahead) — `TS` is probably UTC+8, not UTC. Job 6's `event_time` is the max across books, so every pair lbank is in reports a timestamp 8 h in the future. Confirm the zone, then fix `LbankParser`
- [ ] Flink REST vertex metrics read 0 for every rate on every running job (2026-09-13) — unusable for diagnosis; find out why (metric fetcher vs TaskManager?)
...
- [x] **Rewrite `scripts/warmup.sh` in Go — DONE 2026-09-12** (user request: the shell version took
      ~1 hour at 9 exchanges × 50 markets). Cause was one `docker exec` + JVM per topic across
      ~3010 topics; `warmup/` batches every admin call instead (one `ListTopics`, then
      describe/create/alter in batches of 500 grouped by retention). Also `.env`-configurable,
      retention now RECONCILED on existing topics rather than fixed at creation, postgres read with
      `pgx` from the host instead of `docker exec psql`, and the whole `schemas/*.avsc` directory
      registered instead of a hand-written subject list. `make warmup` is the entry point and both
      old call sites use it. 17 unit tests, 3 mutations killed. See [[project_kafka_topic_strategy]]
- [x] **Restructured + both scripts deleted — 2026-09-12, same day, user request.** The first cut
      was a flat package under `scripts/warmup/` with `.env.example` at the repo root; the user
      rejected it ("high school project") and asked for the `web/` shape. Now a top-level `warmup/`
      module: `main.go` + `internal/{config,domain,postgres,topics,kafka,registry}`, own
      Makefile/README, godotenv + testify like `web/`. `.env.example` lives in `warmup/` and
      `make warmup` copies it to `.env` when missing. `scripts/warmup.sh` and
      `scripts/diagnose-stuck-markets.sh` DELETED. README, both compose comments and the root
      Makefile updated
- [x] **All five retentions set to 1 hour — 2026-09-12, user decision.** In `warmup/.env.example`
...
- [ ] **`make prod-deploy` now needs Go on the deploy server** (opened 2026-09-12). It runs
      ...
      `scripts/install-deps.sh` is where a Go install would belong
...
## flink/normalizer — parser split out of the pair extractor (2026-09-19)

- [x] **`job-parser` added as raw-pipeline job 1** (branch `refactor/standalone-parser-job`,
      started by a teammate in `577f2f8`, finished the same day) — parsing and pair extraction were
      one job, which is two responsibilities and one redeploy surface. `job-parser` consumes
      `^ex[0-9]+-raw$`, runs the nine exchange parsers and emits `ex{id}-parsed-flink` (NEW subject
      `parsed-book-event`); `job-pair-extractor` became job 2 and does nothing but market →
      `pair_id`. `ParsedBookEvent` WRAPS `RawOrderBookEvent` so the two schemas cannot drift; the
      topic is per EXCHANGE because `pair_id` is exactly what job 1 does not know; `parse_in`/
      `parse_out` are their own timing pair so the Kafka hop the split introduced is measurable;
      the no-id drop stayed in job 1 because it reads NiFi's id one line before the operator
      overwrites that field. The teammate's half (Java, schemas, topics, Makefile/Dockerfile/
      compose/warmup/e2e/prometheus wiring) was already complete and green. See [[project_parser]]
- [x] **The surrounding sweep, finished 2026-09-19** — (a) `ex{id}-parsed-flink` was monitored by
      NOTHING; added by hand to `lpa-staleness-exporter/config.yaml` for ex1–ex9 at threshold 30,
      since it has no `pair_id` and so no subscription row to derive it from — without it a dead
      parser shows up only as every per-pair stage below it going stale at once. (b) **Every
      downstream job's number shifted by one** (type-validate 2→3, rebase 3→4, precision 4→5,
      book-build 5→6, aggregate 6→7) and had only been applied in ~8 files; swept across ~100
      (Java javadoc, Go, `exporter.py`, READMEs, `sample-raw-data.md`, swagger). Every `job 1`
      reference was reviewed by hand, because it splits three ways: parsing stays 1,
      pair-resolution/raw-event-minting becomes 2, and the files already rewritten were on the new
      numbering. (c) `README.md` gained `./run-job.sh job-parser` and "6 chained" → 7.
      (d) `sample-raw-data.md` § ex9's fixture path followed the parsers to `job-parser/`.
      (e) the latency-monitor README's sample output showed a 5-stage table; it renders 6.
      (f) `e2e/docs/` regenerated via `e2e/swagger-update.sh`, not hand-edited.
      **Verified: 325 Java tests green (clean run, 8 modules), `go build`/`vet`/`test`/`gofmt`
      clean on e2e + warmup + latency-monitor + orderbook-viewer, and the `job-parser` shaded jar
      packages with the right `Main-Class` carrying its parsers and serde.**
- [ ] **⚠ NOT DEPLOYED and NOT run live.** Deploy is a full one, not a job-only resubmit: `warmup`
      must run first (it registers the NEW `parsed-book-event` subject and creates
      `ex{id}-parsed-flink`), then submit downstream-first with `job-parser` LAST. Prod now needs
      15 of its 16 slots.
- [x] **5 e2e scenarios RUN LIVE against the split and PASS** (2026-09-19, local dev stack):
      **01**-ex1-ws-snapshots-replace-wholesale (plain snapshot feed, whole chain), **34**-ex6-
      sequence-gap (gap → reset → control plane, across the new hop), **36**-ex6-noise-frames
      (the drop rules now span jobs 1 AND 2), **39**-ex8-happy-path (delta feed, dynamic jump; run
      3×). Lineage is asserted structurally on every scenario, so the extra hop is covered by all
      four. Run with `-provision-stack=false` against a stack brought up WITHOUT `down -v`, so
      nothing was wiped — `SCENARIO=<n> go run . -provision-stack=false` from `e2e/`.
      `/jobs/overview` confirmed 7 normalizer jobs RUNNING incl. `normalizer-parser`, and the
      latency monitor on `ex8-p1-orderbook-snapshot-flink` rendered the **6-stage table with
      `1 parse` populated and a real 37–190 ms wait in front of `2 pair-extract`** — the Kafka hop
      the split introduced, measurable exactly as intended.
- [ ] **⚠ The other ~63 scenarios have NOT been run against the split**, and none of the five was
      MUTATION-checked (proven to pass, not proven to bite). A full suite run needs the destructive
      `down -v` the harness does by default.
- [ ] ⚠ In those runs `source` / `end-to-end` printed as large NEGATIVE durations. That is the
      fixtures' fixed exchange timestamps sitting ~117 days in the past, not a pipeline fault — the
      per-stage numbers and every assertion were correct. Worth confirming against a live feed
      before reading those two totals as meaningful on a replayed scenario.
- [ ] **`docker-compose.yml` documents 15 task slots but configures 50.** Pre-existing drift — it
      said 14 against 50 before the split — so it was left alone. Worth one line to settle which
      number is meant.
- [ ] **`.vscode/settings.json` picked up an unrelated `java.jdt.ls.vmargs` line** in `577f2f8`.
      An IDE preference, not part of the refactor; left as the teammate committed it.
