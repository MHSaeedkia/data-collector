...
- [ ] **⚠ `prod-deploy` no longer fits prod** — parallelism now sums to 14 and `docker-compose.prod.yml` has 4 TM × 3 slots = 12. Decide: 4 slots per TM (re-check that TM's memory limit/process.size sizing), a 5th TM, or a prod-only parallelism override
