...
func validate(s scenario.Scenario) error {
	...
	// A control command can only ever name the market the scenario feeds: one
	// scenario produces to one exchange's raw topic, and job 3 builds the command
	// from the event that triggered it. A want that names anything else can never
	// match, so it is worth a 400 rather than a failed run minutes later.
	for i, command := range s.WantControlCommands {
		if command.Action == "" {
			return fmt.Errorf("want_control_commands[%d]: action must not be empty", i)
		}
		// The two triggers job 3 asks on. Anything else — `awaiting_snapshot` in
		...
		if command.Reason != "no_baseline" && command.Reason != "sequence_gap" {
			return fmt.Errorf("want_control_commands[%d]: reason is %q; job 3 only asks on \"no_baseline\" or \"sequence_gap\"",
				i, command.Reason)
		}
