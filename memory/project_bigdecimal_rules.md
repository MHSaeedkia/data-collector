...
So never key price levels by raw string or in a `HashMap<BigDecimal,…>`. The live pattern:
the [[book-builder]] and [[aggregator]] use hash-based Flink `MapState` keyed by the **canonical
string** `new BigDecimal(price).stripTrailingZeros().toPlainString()`. See [[orderbook-aggregation]].
