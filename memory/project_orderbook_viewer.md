...
## Live order book web UI (`orderbook-viewer/`)

**⚠ The service was `web/`, module `orderbook-web`, until 2026-09-13** (renamed, see the last
section). Every `web/…` path and every `orderbook-web…` identifier still written in the sections
below means `orderbook-viewer/…` / `orderbook-viewer…` — the lines someone would actually TYPE
(the run commands, the docker pause repro) were updated; the narrative was left as it was
written.
...
- Fresh consumer group each start (`orderbook-viewer-<unixnano>`) + reset to earliest offset, so the
...
`cd orderbook-viewer && go run .` → http://localhost:3000  (or `go build -o orderbook-viewer . && ./orderbook-viewer`)
...
create a throwaway topic matching the regex, let the app discover it, `docker pause orderbook-viewer`,
...
`kafka`/`orderbook-viewer` would either refuse to start or silently drive production — and **no
...
## 2026-09-13 — the service is `orderbook-viewer` everywhere

The user asked for a more meaningful name than `web`. Offered `orderbook-viewer` (recommended),
`orderbook-ui`, `book-viewer` or keeping `orderbook-web`; they first kept the old name, then chose
**`orderbook-viewer`** — which is what the service is: a read-only live view of the books the
pipeline produces, named like its siblings (`market-subscriptions`, `lpa-staleness-exporter`).

Renamed, all of it: the directory, the **Go module** (so every `orderbook-web/internal/…` import
moved), the binary, the compose service key + `container_name` + `hostname` in both compose files,
the e2e stack's service and project (`orderbook-viewer-e2e`, `ORDERBOOK_VIEWER_E2E_PROJECT`), and
the comments elsewhere that named the old path — `Dockerfile`, `AdjustedOrderBookSerializer`'s
javadoc about job 6's frozen contract, the root README.

### The two things that are not just text

- **The Kafka consumer groups are now `orderbook-viewer-agg-*` / `orderbook-viewer-ex-*`.** The
  prefix is built in `newConsumer`. Nothing in this repo matched the old one (checked
  `monitoring/`, `scripts/`, `Makefile`, `e2e/`, `.vscode/`), but a Grafana panel or an alert on
  the SERVER watching `orderbook-web-agg-*` lag will silently show nothing after this deploys.
  Lag is how the hub freeze was meant to be spotted, so that panel matters.
- **`docker compose up -d --build web` is now `… orderbook-viewer`.** Confirmed with
  `docker compose config --services`.

A stale consumer group name is harmless in Kafka itself: the group name carries a timestamp and a
fresh one is created every start by design, so the old groups simply age out.
