...
/**
 * Splits each job-6 {@link OrderBookSnapshot} into two per-side {@link ExchangeBook}s (asks, bids)
 ...
 * though it would shorten a trace by one hop. It names a job-5 event, which this record never read;
 * stamping it here would make the aggregated record point straight past job 6 at a record it has no
 * relationship with, and the job-6 hop would then appear nowhere in the lineage at all. A level
 * names the snapshot; the snapshot's level of the same price names the event. Each hop, one step.
 *
 * <p>An emitted book always carries both sides; on job 6's reset both sides are empty, which
 * produces two empty ExchangeBooks and drops that exchange from the union. A null side (defensive —
 * job 6 emits both) is treated as empty.
 */
