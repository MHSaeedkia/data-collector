...
/**
 * Tests the second rule every parser shares: NiFi injects {@code sink_id} on the raw payload — the
 ...
 * {@code simulation} is benign and reads as 0 (live). A missing {@code sink_id} is not benign — it
 ...
 * {@code PairExtractFunctionTest.dropsWhenNoSinkId} covers it.
 */
class SinkIdTest {

    private static final String NIFI_SINK_ID = "11111111-1111-4111-8111-111111111111";

    /** Re-serializes a fixture with {@code sink_id} injected at the root, as NiFi does. */
    private static byte[] withSinkId(String fixture, String value) throws Exception {
        ObjectNode root = (ObjectNode) Json.MAPPER.readTree(Fixtures.bytes(fixture));
        root.put("sink_id", value);
    ...
    @DisplayName("the root sink_id becomes source_ids on every event the parser emits")
    @CsvSource({
            "1, ex1-snapshot.json",
            "1, ex1-update.json",
            "2, ex2-snapshot.json",
    ...
    void sinkIdReachesEveryEvent(int exchangeId, String fixture) throws Exception {
        List<ParsedBookEvent> parsed =
                parse(parserFor(exchangeId), withSinkId(fixture, NIFI_SINK_ID));

        assertThat(parsed).allSatisfy(p ->
                assertThat(p.getEvent().getSourceIds()).containsExactly(NIFI_SINK_ID));
    }

    /**
     * A payload with no sink_id still PARSES — it just comes out with no source, which is what the
     ...
     */
    @ParameterizedTest(name = "ex{0} {1}")
    @DisplayName("a payload without sink_id parses but carries no source")
    @CsvSource({
            "1, ex1-snapshot.json",
            "2, ex2-update.json",
            "4, ex4-snapshot.json",
    ...
    void missingSinkIdLeavesNoSource(int exchangeId, String fixture) throws Exception {
    ...
    @Test
    @DisplayName("a blank sink_id is treated as absent, not as a real id")
    void blankSinkIdLeavesNoSource() throws Exception {
        List<ParsedBookEvent> parsed = parse(parserFor(6), withSinkId("ex6-snapshot.json", "   "));

        assertThat(parsed.get(0).getEvent().getSourceIds()).isEmpty();
    }

    /**
     * ex3/wallex carries sink_id in the SAME trailing object as simulation, not as a fourth element —
     * its payload root is an array, so there is no root field to inject into.
     */
    @ParameterizedTest
    @CsvSource({"ex3-buy-depth.json", "ex3-sell-depth.json"})
    @DisplayName("ex3 reads sink_id from the trailing element of its array envelope")
