 ...
 * sequence rules
 * decided in memory/project_raw_pipeline_decision.md; valid events go to the
 * main output, rejects
 * to the {@link #REJECTED} side output (dead-letter). Two rule kinds, selected
 * by what the parser
 ...
 * order by, so
 * out-of-order is detected by <b>event time</b> instead — a null-seq snapshot
 * older than the
 * last accepted event is rejected {@code out_of_order} (an old ex1 REST
 * snapshot replayed
 * after newer WS deltas must not overwrite the newer book). Two exchanges land
 * here: ex3
 * wallex (never sends updates) and the ex1 nobitex REST snapshot (a resync
 * whose Centrifugo
 * offset is unknown). An accepted one sets {@code baselinePending} so that the
 * next update on
 ...
 * dropped — reject {@code stale_or_duplicate} if
 * {@code sequence_id <= lastSeq}. Otherwise it
 * re-syncs the book: store {@code lastSeq}, clear
 * {@code awaitingSnapshot}.</li>
 * <li><b>Update</b> ({@code type == "update"}, delta feeds ex1/ex6/ex8): needs
 * a baseline and a
 * contiguous sequence. No baseline yet → {@code no_baseline}; still waiting to
 * re-sync after
 * a gap → {@code awaiting_snapshot};
 * {@code sequence_id == lastSeq + sequence_jump} → valid;
 * {@code sequence_id <= lastSeq} → {@code stale_or_duplicate}; any other
 * forward jump is a
 * gap → {@code sequence_gap} + set {@code awaitingSnapshot} (every update
 * rejected until the
 * next snapshot re-syncs).</li>
 * </ul>
 *
 * State per key: {@code lastSeq} (last accepted sequence id) and
 * {@code awaitingSnapshot}. Topics
 * are single-partition so per-key order holds; no checkpointing configured
 * (cold-start gap shared
 * with the rest of the platform — book is unvalidated until the first snapshot
 * after a restart).
 */
public class TypeValidateFunction
    ...
    /**
     * Event {@code type} of the synthetic reset marker emitted onto the main stream
     * on a gap. Job 5
     * turns it into an emptied book so the exchange drops out of the aggregated
     * view instead of
     ...
     * episode — a
     * second call before the flag is cleared is a no-op, so NiFi doesn't get
     * flooded with
     * duplicate requests while every subsequent update keeps rejecting on the same
     * condition.
     */
    ...
    /**
     * Emits a synthetic {@link #RESET} marker on the main stream carrying the gap
     * event's identity
     * and event time but no book (null sides, null sequence). Reached only on the
     * not-awaiting →
     * awaiting transition, so it fires exactly once per gap episode — subsequent
     * updates return at
     * the {@code awaitingSnapshot} reject above. A fresh {@link
     * io.tibobit.normalizer.model.PipelineTimings} (not the gap event's) is used so
     * stamping
     * {@code type_validate_out} here does not leak onto the event that is still
     * being dead-lettered.
     *
     * <p>
     * {@code simulation} IS inherited from the gap event: the marker stands in for
     * that
     * exchange's stream, so an emptied simulation book must not come back out
     * flagged as live.
     *
     * <p>
     * Lineage is NOT inherited — it is derived. The marker is a new record caused
     * by the gap
     * event, so the gap event is its source and the marker gets an id of its own.
     * The gap event
     * is simultaneously dead-lettered, so its one id legitimately appears as the
     * parent of two
     * different records.
     */
