 ...
 *
 * <p>
 * Expected prices are spelled out as literals rather than recomputed from the
 * rates: a test that repeats the implementation's formula agrees with the
 * implementation by construction and proves nothing. These were worked out
 * independently and are what the job must publish.
 *
 * <p>
 * {@link #commissionFunction}/{@link #profitFunction}/{@link #slippageFunction}
 * open with an EMPTY lookup unless a test needs specific rows — an empty map
 * means every level falls back to {@code DEFAULT_PERCENT} (0.35%/0.1%/1%),
 * which is deliberately identical to the pre-DB hardcoded constants, so every
 * test that predates the DB read keeps its original literal expectations
 * unchanged.
 */
class AdjustmentFunctionsTest {

    /**
     * 62650.00 chosen for readable arithmetic: x1.0035 lands exactly on
     * 62869.275.
     */
    ...
    private static BuySellCommissionFunction commissionFunction(Map<String, AdjustmentFactors> rows) throws Exception {
        BuySellCommissionFunction fn = new BuySellCommissionFunction(new RefreshingLookup<>(() -> rows, 60_000L));
        fn.open(null);
        return fn;
    }
    ...
    private static AdjustedOrderBook wholeChain(String side) throws Exception {
        return slippageFunction(Map.of())
                .map(profitFunction(Map.of())
                        .map(commissionFunction(Map.of()).map(book(side))));
    }
    ...
    @Test
    void commissionRaisesAskPricesAndLowersBidPrices() throws Exception {
        ...
        assertThat(prices(commissionFunction(Map.of()).map(book("asks"))))
                .containsExactly("62869.275", "1003.5");
        assertThat(prices(commissionFunction(Map.of()).map(book("bids"))))
    ...
    void everyStageMovesAsksUpAndBidsDown() throws Exception {
        ...
        for (String side : List.of("asks", "bids")) {
            ...
            assertThat(new BigDecimal(prices(commissionFunction(Map.of()).map(book(side))).get(1)))
    ...
    void eachStageAppliesItsOwnRate() throws Exception {
        assertThat(prices(commissionFunction(Map.of()).map(book("asks"))).get(1)).isEqualTo("1003.5");
    ...
    void eachStageRecordsTheRateItApplied() throws Exception {
        assertThat(commissionFunction(Map.of()).map(book("asks")).getLevels().get(1).getBuySellCommissionPercent()).isEqualTo("0.35");
    ...
    @Test
    void aStageLeavesTheOtherTwoRatesUntouched() throws Exception {
        AdjustedOrderBook only = commissionFunction(Map.of()).map(book("asks"));
        AdjustedLevel level = only.getLevels().get(1);

        assertThat(level.getBuySellCommissionPercent()).isEqualTo("0.35");
        // profit/slippage default "0" on every level until their own stage runs.
        assertThat(level.getOurProfitPercent()).isEqualTo("0");
        assertThat(level.getSlippagePercent()).isEqualTo("0");
    }
    ...
    /**
     * The direct consequence of sizing every amount off the original: no stage
     * sees another's output, so addition commutes and the chain order cannot
     * change the numbers. It did under the first (compounding) implementation,
     * which is why this is worth pinning — if it ever fails, someone has
     * reintroduced a dependency between the stages.
     */
    @Test
    void reorderingTheStagesCannotChangeTheResult() throws Exception {
        AdjustedOrderBook forward = slippageFunction(Map.of())
                .map(profitFunction(Map.of()).map(commissionFunction(Map.of()).map(book("asks"))));
        AdjustedOrderBook reversed = commissionFunction(Map.of())
    ...
    /**
     * Each stage's contribution is a fixed money amount off the base, not a
     * share of the running price.
     */
    @Test
    void eachStagesAmountIsSizedOffTheOriginalPrice() throws Exception {
        AdjustedOrderBook afterFirst = commissionFunction(Map.of()).map(book("asks"));
    ...
    void theChainReportsAllThreeRates() throws Exception {
        ...
        AdjustedLevel level = out.getLevels().get(1);

        assertThat(level.getBuySellCommissionPercent()).isEqualTo("0.35");
        assertThat(level.getOurProfitPercent()).isEqualTo("0.1");
        assertThat(level.getSlippagePercent()).isEqualTo("1");
    }

    // ---- all three rates looked up per (exchange, pair) — the whole point of the DB move ---------
    /**
     * The scenario the redesign exists for: one book, two exchanges, two
     * different DB rows. A flat record-level rate could never have produced
     * this — exchange 6 and exchange 8 must each get their OWN rate even though
     * both levels are in the same p3-asks book.
     */
    @Test
    void differentExchangesInTheSameBookGetDifferentRates() throws Exception {
        Map<String, AdjustmentFactors> rows = Map.of(
                "6|3", new AdjustmentFactors(new BigDecimal("1"), new BigDecimal("2"), BigDecimal.ZERO),
                "8|3", new AdjustmentFactors(new BigDecimal("3"), new BigDecimal("4"), BigDecimal.ZERO));
    ...
    /**
     * Commission gets the identical per-exchange treatment as profit/slippage.
     */
    @Test
    void commissionAlsoVariesPerExchangeInTheSameBook() throws Exception {
        Map<String, AdjustmentFactors> rows = Map.of(
                "6|3", new AdjustmentFactors(BigDecimal.ZERO, BigDecimal.ZERO, new BigDecimal("2")),
                "8|3", new AdjustmentFactors(BigDecimal.ZERO, BigDecimal.ZERO, new BigDecimal("5")));

        AdjustedOrderBook out = commissionFunction(rows).map(book("asks"));

        assertThat(out.getLevels().get(0).getBuySellCommissionPercent()).isEqualTo("2");
        assertThat(out.getLevels().get(1).getBuySellCommissionPercent()).isEqualTo("5");
        // 62650.00 x 1.02 = 63903.00, trailing zeros stripped by Decimals.canonicalize
        assertThat(out.getLevels().get(0).getPrice()).isEqualTo("63903");
    }

    /**
     * A level whose (exchange, pair) has no exchange_markets row falls back to
     * the old constant.
     */
    @Test
    void aMissingRowFallsBackToTheDefaultPercent() throws Exception {
        Map<String, AdjustmentFactors> rows = Map.of(
                "6|3", new AdjustmentFactors(new BigDecimal("5"), new BigDecimal("9"), BigDecimal.ZERO));
    ...
    /**
     * Same fallback for commission: a missing row must not silently charge 0%.
     */
    @Test
    void aMissingRowFallsBackToTheDefaultCommissionPercent() throws Exception {
        Map<String, AdjustmentFactors> rows = Map.of(
                "6|3", new AdjustmentFactors(BigDecimal.ZERO, BigDecimal.ZERO, new BigDecimal("2")));
        // No "8|3" row.
        ...
        assertThat(out.getLevels().get(0).getBuySellCommissionPercent()).isEqualTo("2");
        assertThat(out.getLevels().get(1).getBuySellCommissionPercent()).isEqualTo(
                BuySellCommissionFunction.DEFAULT_PERCENT.toPlainString());
    }
    ...
    /**
     * {@code AdjustedOrderBook.from} copies the levels, so the stages mutating
     * in place cannot write through to the record the deserializer produced.
     * Sharing them would corrupt the input of anything that later reads the
     * aggregated book alongside this chain.
     */
    @Test
    ...
    /**
     * The base must survive every stage, or the second and third would size off
     * a moved price.
     */
    @Test
