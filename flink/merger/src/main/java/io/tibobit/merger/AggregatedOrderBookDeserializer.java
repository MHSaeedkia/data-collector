...
public class AggregatedOrderBookDeserializer implements DeserializationSchema<AggregatedOrderBook> {
    ...
    private static List<AggregatedLevel> levels(Object value) {
        ...
        for (Object element : raw) {
            ...
            levels.add(new AggregatedLevel(
    ...
    /**
     * Avro hands back {@link org.apache.avro.util.Utf8} for strings (and an EnumSymbol for
     * {@code side}), not {@link String}. A Utf8 kept as-is compares unequal to every String while
     * still printing correctly in a log — so a price would never match its own map key and a
     * lineage id would fail every assertion, with nothing visibly wrong. Convert at the boundary.
     */
