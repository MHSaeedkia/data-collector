...
func TestRecreatedTopicIsPurgedAndRediscovered(t *testing.T) {
	...
	t.Cleanup(func() { s.unpauseQuietly("orderbook-viewer") })
	...
	logs := s.logsSince("orderbook-viewer", mark)
	...
	t.Logf("orderbook-viewer log after unpause:\n%s", tail(logs, 40))
