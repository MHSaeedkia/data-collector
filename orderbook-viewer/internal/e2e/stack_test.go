...
// project-scoped and can never be the real `kafka` or `orderbook-viewer`.
...
func (s *stack) startWeb() {
	s.t.Helper()
	s.compose("up", "-d", "--build", "orderbook-viewer")
	s.waitForLog("orderbook-viewer", time.Time{}, "Order book UI:", 5*time.Minute)
}
...
func (s *stack) dumpLogs() {
	s.t.Helper()
	for _, svc := range []string{"orderbook-viewer", "kafka"} {
...
func (s *stack) produceUntilLog(topic, msg string, since time.Time, want string, timeout time.Duration) {
	...
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		if strings.Contains(s.logsSince("orderbook-viewer", since), want) {
			return
		}
	...
	s.t.Fatalf("%q never appeared in web logs within %s despite producing to %s\n--- logs ---\n%s",
		want, timeout, topic, tail(s.logsSince("orderbook-viewer", since), 40))
}
